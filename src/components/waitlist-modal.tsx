...
/** Minimal sold-out capture: name + mobile only, no email, no payment.
 *  Renders as a filled primary button so it drops into the CTA slot that
 *  "Register Now" occupies when seats are available. */
export function WaitlistModal({ courseId, courseScheduleId, courseTitle }: Props) {
  const { dict } = useLanguage();
  const t = dict.waitlistModal;
      ...
      {open && (
        ...
        >
          <div
            ...
            className="max-h-[92vh] w-full max-w-[440px] overflow-y-auto rounded-t-[22px] bg-surface p-5 shadow-2xl sm:rounded-[22px] sm:p-7"
          >
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
                <h3 id="waitlist-modal-title" className="text-lg font-bold">{t.title}</h3>
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-4">
              <div>
                <label htmlFor="wl-name" className="mb-1.5 block text-[13.5px] font-semibold">{t.fullName}</label>
                <input
                  id="wl-name"
                  value={fullName}
              ...
              <div>
                <label htmlFor="wl-phone" className="mb-1.5 block text-[13.5px] font-semibold">{t.phone}</label>
                <input
                  id="wl-phone"
                  value={phone}
