...
export async function POST(request: Request) {
  ...
  const interest = clean(body.interest, 500);
  ...
  if (!isSupabaseConfigured) {
    return NextResponse.json(
      { error: "The reservation form isn't connected yet — please email us directly." },
      { status: 503 }
    );
  }
  ...
  if (error) {
    console.error("seat_reservations insert error:", error.message);
